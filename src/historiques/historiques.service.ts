...
export class HistoriquesService {
    constructor(
        ...
        @Inject(forwardRef(() => TransfertService)) private readonly transferService: TransfertService,
    ...
    async getTransfertHistoriqueFilters(options: GetHistoriqueFilterDto): Promise<HistoriqueFilterDto> {        
    ...
    async getOneHistorique(id: string){
        const historique = await this.repository.findOne({where: {id}, relations: ["sender", "receiver"]})
        const transactionType = historique.transactionType
        if(transactionType === "TRANSFERT"){
            const getTransfertDetails = await this.transferService.getTransferByReference(historique.referenceTransaction)
            const receiver = await this.userService.getUserById(historique.receiverIdentifiant)
            return {
                ...getTransfertDetails,
                receiver,
                transactionType
            ...
            const getPaiementDetails = await this.paiementService.getPaymentByReference(historique.referenceTransaction)
            const receiver = await this.userService.getUserById(historique.receiverIdentifiant)
            return {
                ...getPaiementDetails,
                receiver,
