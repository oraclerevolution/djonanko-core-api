...
import {
  MakePaiementDto,
  makePaiementResultDto,
} from './dto/make-paiement.dto';
import {
  CreateHistoriqueDto,
  CreateHistoriqueResultDto,
} from 'src/historiques/dto/create-historique.dto';
...
export class PaiementService {
  constructor(
    ...
    @Inject(forwardRef(() => HistoriquesService))
  ...
  async paymentInitializer(payload: MakePaiementDto): Promise<PaymentInitDto> {
    ...
    const getSenderInfos = await this.userService.getUserByPhoneNumber(
      senderPhoneNumber,
    );
  ...
  async paymentDebit(
    ...
    const reservation =
      await this.compteReservationService.createCompteReservation({
        amount: amount,
        fees: fees,
  ...
  async sendPayment(
    ...
    const updateReceiverBalance = await this.userService.updateUser(
      getReceiverInfos.id,
      {
    ...
    if (updateReceiverBalance.affected === 1) {
      ...
      if (debit.affected === 1) {
        ...
        if (credit) {
          ...
          await this.compteReservationService.updateCompteReservation(
            reservation.id,
            {
  ...
  async makeAbonnement(payload: MakeAbonnementDto) {
    ...
    const compteCollect = await this.userService.getUserByPhoneNumber(
      this.configService.get<string>('COMPTE_COLLECTE'),
    );
    ...
    if (historique) {
      if (payment) {
        ...
        if (transaction) {
          ...
          const debitSender = await this.userService.updateUser(
            getSenderInfos.id,
            {
              solde: (
                parseInt(getSenderInfos.solde) - parseInt(amount)
          ...
          if (
            parseInt(getSenderInfos.solde) - parseInt(amount) < 0 ||
            parseInt(getSenderInfos.solde) < parseInt(amount)
          ) {
            //update payment status
            await this.repository.update(payment.id, {
            ...
            //update historique status
            await this.historiqueService.updateHistorique(
          ...
          if (debitSender.affected === 1) {
            ...
            if (reservation) {
              const creditCompteReservation =
                await this.userService.getUserByPhoneNumber(
                  this.configService.get<string>('COMPTE_RESERVATION'),
                );
              const credit = await this.userService.updateUser(
              ...
              if (credit.affected === 1) {
                ...
                if (creditCollectAccount) {
                  ...
                  const debitReservation = await this.userService.updateUser(
                    creditCompteReservation.id,
                    {
                      solde: (
  ...
  async paymentRequest(payload: PaymentRequestDto): Promise<Paiement> {
    const { amount, senderPhoneNumber, receiverPhoneNumber } = payload;
    const debitedUser = await this.userService.getUserByPhoneNumber(
      senderPhoneNumber,
    );
    const creditedUser = await this.userService.getUserByPhoneNumber(
      receiverPhoneNumber,
    );
  ...
  async validatePaymentRequest(
    ...
    try {
      ...
      const paymentRequestHistory =
        await this.historiqueService.getHistoriqueByReference(reference);
      const debitedUser = await this.userService.getUserByPhoneNumber(
      ...
      if (reservation) {
        const compteReservation = await this.userService.getUserByPhoneNumber(
          this.configService.get<string>('COMPTE_RESERVATION'),
        );
      ...
      if (updateReceiverBalance.affected === 1) {
        ...
        if (updateReservationBalance.affected === 1) {
          ...
          if (updateReservation.affected === 1) {
            ...
            const compteCollecte = await this.userService.getUserByPhoneNumber(
              this.configService.get<string>('COMPTE_COLLECTE'),
            );
            if (compteCollecte) {
              await this.userService.updateUser(compteCollecte.id, {
                solde: (
                  parseInt(compteCollecte.solde) + parseInt(paymentRequest.fees)
                ).toString(),
              });
            }
          }
  ...
  async getPaiementByReceiverNumber(
    receiverPhoneNumber: string,
  ): Promise<Paiement[]> {
    return await this.repository.find({
      where: {
  ...
  async createpaiement(
    paiement: MakePaiementDto,
    getSenderInfos: User,
  ): Promise<makePaiementResultDto> {
  ...
  async createHistorique(
    historique: CreateHistoriqueDto,
  ): Promise<CreateHistoriqueResultDto> {
